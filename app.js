...
// ================================================================
// ALERTA DE ESTOQUE CRÍTICO — Dashboard
// ================================================================
function AlertaEstoqueCritico({ estoque, onNavigate }) {
  const [fechado, setFechado] = useState(false);
  if (fechado) return null;

  const criticos = estoque.filter(e => e.quantidade <= (e.minimo || 0));
  if (criticos.length === 0) return null;

  const urgentes = criticos.filter(e => e.quantidade === 0);
  const cor = urgentes.length > 0 ? C.red : C.yellow;

  return (
    <div style={{
      background: cor+"14", border:`1px solid ${cor}55`,
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
        <div style={{ display:"flex", alignItems:"flex-start", gap:8 }}>
          ...
          <div>
            <div style={{ color:cor, fontWeight:900, fontSize:13 }}>
              {urgentes.length > 0
                ? `${urgentes.length} item(ns) ZERADO(S) no estoque!`
                : `${criticos.length} item(ns) abaixo do mínimo`}
            </div>
            {urgentes.length > 0 && criticos.length > urgentes.length && (
              <div style={{ color:C.yellow, fontSize:11, marginTop:2 }}>
                + {criticos.length - urgentes.length} item(ns) abaixo do mínimo
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
        {criticos.slice(0, 4).map(item => {
          ...
          const corItem = zerado ? C.red : C.yellow;
          return (
            <div key={item.id} style={{
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ color:C.text, fontWeight:700, fontSize:12, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                  📦 {item.nome}
                </div>
                <div style={{ color:C.muted, fontSize:11, marginTop:1 }}>{item.categoria || "—"}</div>
              </div>
              <div style={{ textAlign:"right", flexShrink:0, marginLeft:10 }}>
                <span style={{ background:corItem+"22", color:corItem, fontWeight:900, fontSize:12, padding:"2px 8px", borderRadius:5 }}>
                  {item.quantidade} {item.unidade || "un"}
                </span>
                <div style={{ color:C.muted, fontSize:10, marginTop:2 }}>mín: {item.minimo || 0}</div>
              </div>
            </div>
          );
        })}
        {criticos.length > 4 && (
          <div style={{ color:C.muted, fontSize:11, textAlign:"center", paddingTop:2 }}>
            + {criticos.length - 4} item(ns) crítico{criticos.length - 4 > 1 ? "s" : ""}...
          </div>
        )}
      </div>
    </div>
...
function Dashboard({ ordens, estoque, caixa, agendamentos=[], onNavigate }) {
  ...
  return (
    <div className="fade-in" style={{ display:"flex", flexDirection:"column", gap:8 }}>
      ...
      <AlertaEstoqueCritico estoque={estoque} onNavigate={onNavigate}/>
...
const imprimir = async () => {
    ...
    const linhasServicos = (os.servicos||[]).map(function(sv) {
      ...
      const bruto = (parseFloat(sv.valorUnit)||0)*(parseInt(sv.quantidade)||0);
      const desc = parseFloat(sv.desconto)||0;
      const descVal = sv.descontoTipo==="%" ? bruto*(desc/100) : Math.min(desc, bruto);
      const liq = Math.max(0, bruto - descVal);
      const subHtml = desc > 0
        ? "<span style='text-decoration:line-through;color:#999;font-size:10px'>R$ "+bruto.toFixed(2)+"</span> R$ "+liq.toFixed(2)
        : "R$ "+liq.toFixed(2);
      const descHtml = desc > 0
        ? " <span style='background:#fee2e2;color:#dc2626;font-size:9px;padding:1px 4px;border-radius:3px'>-"+(sv.descontoTipo==="%"?desc+"%":"R$ "+desc)+"</span>"
        : "";
      return "<tr><td>" + sv.descricao + descHtml + "</td><td>" + tipo + "</td>"
           + "<td style='text-align:center'>" + sv.quantidade + "</td>"
           + "<td style='text-align:right'>R$ " + vUnit + "</td>"
           + "<td style='text-align:right'>" + subHtml + "</td></tr>";
    }).join("");
    const subTotal = (os.servicos||[]).reduce(function(a,s){
      var b=(parseFloat(s.valorUnit)||0)*(parseInt(s.quantidade)||0);
      var d=parseFloat(s.desconto)||0;
      var dv=s.descontoTipo==="%"?b*(d/100):Math.min(d,b);
      return a+Math.max(0,b-dv);
    }, 0);
    const dg = parseFloat(os.descontoGlobal)||0;
    const dgVal = os.descontoGlobalTipo==="%" ? subTotal*(dg/100) : Math.min(dg, subTotal);
    const totalFinal = Math.max(0, subTotal - dgVal);
    const totalFmt = totalFinal.toFixed(2);
    const descontoGlobalHtml = dg > 0
      ? "<tr style='background:#fff7ed'><td colspan='4' style='text-align:right;color:#555'>Subtotal dos itens</td><td style='text-align:right;color:#555'>R$ "+subTotal.toFixed(2)+"</td></tr>"
        + "<tr style='background:#fef2f2'><td colspan='4' style='text-align:right;color:#dc2626'>Desconto global ("+(os.descontoGlobalTipo==="%"?dg+"%":"R$ "+dg)+")</td><td style='text-align:right;color:#dc2626'>− R$ "+dgVal.toFixed(2)+"</td></tr>"
      : "";
        ...
        <div style={{ background:"#141416", borderRadius:8, overflow:"hidden", marginBottom:12 }}>
          ...
          {(os.servicos||[]).map((sv,i) => {
            ...
            <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"8px 12px", borderBottom:"1px solid #1e1e22", fontSize:12 }}>
              ...
              <div style={{ textAlign:"right" }}>
                {desc > 0 && <div style={{ color:"#ef4444", fontSize:11, textDecoration:"line-through" }}>R$ {bruto.toFixed(2)}</div>}
                <div style={{ color:"#f97316", fontWeight:700 }}>R$ {liq.toFixed(2)}</div>
              </div>
            </div>
            );
          })}
          {(() => {
            const subTotal = (os.servicos||[]).reduce((a,s) => {
              const b=(parseFloat(s.valorUnit)||0)*(parseInt(s.quantidade)||0);
              const d=parseFloat(s.desconto)||0;
              const dv=s.descontoTipo==="%"?b*(d/100):Math.min(d,b);
              return a+Math.max(0,b-dv);
            }, 0);
            const dg = parseFloat(os.descontoGlobal)||0;
            const dgVal = os.descontoGlobalTipo==="%" ? subTotal*(dg/100) : Math.min(dg, subTotal);
            ...
            return (
              <>
                {dg > 0 && (
                  <div style={{ display:"flex", justifyContent:"space-between", padding:"6px 12px", background:"#1a0c00" }}>
                    <span style={{ color:"#fb923c", fontSize:12 }}>Subtotal</span>
                    <span style={{ color:"#fb923c", fontSize:12 }}>R$ {subTotal.toFixed(2)}</span>
                  </div>
                )}
                {dg > 0 && (
                  <div style={{ display:"flex", justifyContent:"space-between", padding:"6px 12px", background:"#1a0000" }}>
                    <span style={{ color:"#ef4444", fontSize:12 }}>Desconto global ({os.descontoGlobalTipo==="%" ? dg+"%" : "R$ "+dg})</span>
                    <span style={{ color:"#ef4444", fontSize:12 }}>− R$ {dgVal.toFixed(2)}</span>
                  </div>
                )}
                <div style={{ display:"flex", justifyContent:"space-between", padding:"10px 12px", background:"#431407" }}>
                  ...
                  <span style={{ color:"#fff", fontWeight:900, fontSize:16 }}>R$ {totalFinal.toFixed(2)}</span>
                </div>
              </>
            );
          })()}
...
const oficina = await carregarOficina();
    ...
    const subTotalOrc = (orc.servicos||[]).reduce(function(a,s){
      var b=(parseFloat(s.valorUnit)||0)*(parseInt(s.quantidade)||0);
      var d=parseFloat(s.desconto)||0;
      var dv=s.descontoTipo==="%"?b*(d/100):Math.min(d,b);
      return a+Math.max(0,b-dv);
    }, 0);
    const dgOrc = parseFloat(orc.descontoGlobal)||0;
    const dgOrcVal = orc.descontoGlobalTipo==="%" ? subTotalOrc*(dgOrc/100) : Math.min(dgOrc, subTotalOrc);
    const totalFinal = Math.max(0, subTotalOrc - dgOrcVal);
    const totalFmt  = totalFinal.toFixed(2);
    const descontoGlobalOrcHtml = dgOrc > 0
      ? "<tr style='background:#fff7ed'><td colspan='4' style='text-align:right;color:#555'>Subtotal dos itens</td><td style='text-align:right;color:#555'>R$ "+subTotalOrc.toFixed(2)+"</td></tr>"
        + "<tr style='background:#fef2f2'><td colspan='4' style='text-align:right;color:#dc2626'>Desconto global ("+(orc.descontoGlobalTipo==="%"?dgOrc+"%":"R$ "+dgOrc)+")</td><td style='text-align:right;color:#dc2626'>− R$ "+dgOrcVal.toFixed(2)+"</td></tr>"
      : "";
    ...
    <Modal title={`📋 Orçamento — ${orc.numero}`} onClose={onClose} wide>
      ...
      <div style={{ background:"#0f0f10", border:"1px solid #1e1e22", borderRadius:10, padding:16, marginBottom:16 }}>
        ...
        <div style={{ background:"#141416", borderRadius:8, overflow:"hidden", marginBottom:12 }}>
          ...
          {(() => {
            const subTotalOrc = (orc.servicos||[]).reduce((a,s) => {
              const b=(parseFloat(s.valorUnit)||0)*(parseInt(s.quantidade)||0);
              const d=parseFloat(s.desconto)||0;
              const dv=s.descontoTipo==="%"?b*(d/100):Math.min(d,b);
              return a+Math.max(0,b-dv);
            }, 0);
            const dg = parseFloat(orc.descontoGlobal)||0;
            const dgVal = orc.descontoGlobalTipo==="%" ? subTotalOrc*(dg/100) : Math.min(dg, subTotalOrc);
            const totalFinal = Math.max(0, subTotalOrc - dgVal);
            return (
              <>
                {dg > 0 && (
                  <div style={{ display:"flex", justifyContent:"space-between", padding:"6px 12px", background:"#1a0c00" }}>
                    <span style={{ color:"#fb923c", fontSize:12 }}>Subtotal</span>
                    <span style={{ color:"#fb923c", fontSize:12 }}>R$ {subTotalOrc.toFixed(2)}</span>
                  </div>
                )}
                {dg > 0 && (
                  <div style={{ display:"flex", justifyContent:"space-between", padding:"6px 12px", background:"#1a0000" }}>
                    <span style={{ color:"#ef4444", fontSize:12 }}>Desconto global ({orc.descontoGlobalTipo==="%" ? dg+"%" : "R$ "+dg})</span>
                    <span style={{ color:"#ef4444", fontSize:12 }}>− R$ {dgVal.toFixed(2)}</span>
                  </div>
                )}
...
function OrdemServico({ ordens, setOrdens, estoque, setEstoque, caixa, setCaixa, orcamentos, setOrcamentos, logado, pendenciaEdicao, onConsumirPendencia }) {
  ...
  useEffect(() => {
    ...
    if (pendenciaEdicao.tipo === "OS") {
      setModal(pendenciaEdicao.dados);
  ...
  const initOS = () => ({
    ...
    clienteNome:"", clienteTel:"", clienteCpfCnpj:"",
  ...
  const salvarORC = async () => {
    ...
    setSalvandoORC(true);
    try {
      const subTotal = totalOS(novaORC.servicos);
      const dg = parseFloat(novaORC.descontoGlobal)||0;
      const dgVal = novaORC.descontoGlobalTipo==="%" ? subTotal*(dg/100) : Math.min(dg, subTotal);
      const total = Math.max(0, subTotal - dgVal);
  ...
  const totalOS = (sv) => sv.reduce((a,s) => {
    const bruto = (parseFloat(s.valorUnit)||0)*(parseInt(s.quantidade)||0);
    const desc = parseFloat(s.desconto)||0;
    const descVal = s.descontoTipo==="%" ? bruto*(desc/100) : Math.min(desc, bruto);
    return a + Math.max(0, bruto - descVal);
  }, 0);

  // Helper para calcular subtotal líquido de um item
  const subtotalItem = (s) => {
    const bruto = (parseFloat(s.valorUnit)||0)*(parseInt(s.quantidade)||0);
    const desc = parseFloat(s.desconto)||0;
    const descVal = s.descontoTipo==="%" ? bruto*(desc/100) : Math.min(desc, bruto);
    return Math.max(0, bruto - descVal);
  };
  ...
  const salvarOS = async () => {
    ...
    setSalvando(true);
    try {
      const subTotal = totalOS(novaOS.servicos);
      const dg = parseFloat(novaOS.descontoGlobal)||0;
      const dgVal = novaOS.descontoGlobalTipo==="%" ? subTotal*(dg/100) : Math.min(dg, subTotal);
      const total = Math.max(0, subTotal - dgVal);
  ...
  const salvarEdicaoORC = async () => {
    ...
    setSalvandoORC(true);
    try {
      const subTotal = totalOS(modalEditarORC.servicos);
      const dg = parseFloat(modalEditarORC.descontoGlobal)||0;
      const dgVal = modalEditarORC.descontoGlobalTipo==="%" ? subTotal*(dg/100) : Math.min(dg, subTotal);
      const total = Math.max(0, subTotal - dgVal);
  ...
  const salvarEdicaoOS = async () => {
    ...
    setSalvando(true);
    try {
      const subTotal = totalOS(modalEditar.servicos);
      const dg = parseFloat(modalEditar.descontoGlobal)||0;
      const dgVal = modalEditar.descontoGlobalTipo==="%" ? subTotal*(dg/100) : Math.min(dg, subTotal);
      const total = Math.max(0, subTotal - dgVal);
          ...
          {step===0 && (
            <div>
              ...
              <Field label="CPF / CNPJ">
                <Inp placeholder="000.000.000-00 ou 00.000.000/0000-00" value={novaOS.clienteCpfCnpj||""} onChange={e=>upOS("clienteCpfCnpj",e.target.value)}/>
              </Field>
            </div>
          )}
            ...
            <div>
              {novaOS.servicos.map((sv,i) => (
                <div key={i} style={{ background:C.panel, borderRadius:10, padding:12, marginBottom:10, border:`1px solid ${C.border}` }}>
                  ...
                  <div className="form-grid-2" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop:8 }}>
                    <Field label="Desconto por Item">
                      <div style={{ display:"flex", gap:4 }}>
                        <select value={sv.descontoTipo||"R$"} onChange={e=>upSvc(i,"descontoTipo",e.target.value)}
                          style={{ ...css.input, width:56, padding:"9px 6px", cursor:"pointer" }}>
                          <option value="R$">R$</option>
                          <option value="%">%</option>
                        </select>
                        <Inp type="number" min="0" placeholder="0" value={sv.desconto||""} onChange={e=>upSvc(i,"desconto",e.target.value)} style={{ flex:1 }}/>
                      </div>
                    </Field>
                    <div style={{ display:"flex", flexDirection:"column", justifyContent:"flex-end", paddingBottom:2 }}>
                      {sv.valorUnit ? (() => {
                        ...
                        return (
                          <div style={{ textAlign:"right" }}>
                            {desc>0 && <div style={{ color:C.red, fontSize:11, textDecoration:"line-through" }}>R$ {bruto.toFixed(2)}</div>}
                            <div style={{ color:"#fb923c", fontSize:13, fontWeight:700 }}>R$ {liq.toFixed(2)}</div>
                          </div>
                        );
                      })() : null}
                    </div>
              ...
              {/* Desconto Global da OS */}
              <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, padding:"12px 14px", marginTop:14 }}>
                <div style={{ color:C.muted, fontSize:11, fontWeight:700, letterSpacing:1, marginBottom:8 }}>🏷️ DESCONTO GLOBAL DA OS</div>
                <div style={{ display:"flex", gap:4 }}>
                  <select value={novaOS.descontoGlobalTipo||"R$"} onChange={e=>upOS("descontoGlobalTipo",e.target.value)}
                    style={{ ...css.input, width:56, padding:"9px 6px", cursor:"pointer" }}>
                    <option value="R$">R$</option>
                    <option value="%">%</option>
                  </select>
                  <Inp type="number" min="0" placeholder="0" value={novaOS.descontoGlobal||""}
                    onChange={e=>upOS("descontoGlobal",e.target.value)} style={{ flex:1 }}/>
                </div>
              </div>

              {/* Total com desconto global */}
              {(() => {
                const subTotal = totalOS(novaOS.servicos);
                const dg = parseFloat(novaOS.descontoGlobal)||0;
                const dgVal = novaOS.descontoGlobalTipo==="%" ? subTotal*(dg/100) : Math.min(dg, subTotal);
                const totalFinal = Math.max(0, subTotal - dgVal);
                return (
                  <div style={{ background:"#431407", borderRadius:10, padding:"14px 16px", marginTop:8 }}>
                    {dg > 0 && (
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                        <span style={{ color:"#fb923c", fontSize:12 }}>Subtotal dos itens</span>
                        <span style={{ color:"#fb923c", fontSize:12 }}>R$ {subTotal.toFixed(2)}</span>
                      </div>
                    )}
                    {dg > 0 && (
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                        <span style={{ color:C.red, fontSize:12 }}>Desconto global</span>
                        <span style={{ color:C.red, fontSize:12 }}>− R$ {dgVal.toFixed(2)}</span>
                      </div>
                    )}
                    <div style={{ display:"flex", justifyContent:"space-between" }}>
                      <span style={{ color:"#fdba74", fontWeight:700 }}>TOTAL</span>
                      <span style={{ color:"#fff", fontWeight:900, fontSize:18 }}>R$ {totalFinal.toFixed(2)}</span>
                    </div>
                  </div>
                );
              })()}
          ...
          <div style={css.card}>
            ...
            {(modal.servicos||[]).map((sv,i) => {
              ...
              <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"8px 0", borderBottom:`1px solid ${C.border}` }}>
                ...
                <div style={{ textAlign:"right" }}>
                  {desc > 0 && <div style={{ color:C.red, fontSize:11, textDecoration:"line-through" }}>R$ {bruto.toFixed(2)}</div>}
                  <span style={{ color:C.accent, fontWeight:700 }}>R$ {liq.toFixed(2)}</span>
                </div>
              </div>
              );
            })}
            {(() => {
              const subTotal = (modal.servicos||[]).reduce((a,s) => {
                const b=(parseFloat(s.valorUnit)||0)*(parseInt(s.quantidade)||0);
                const d=parseFloat(s.desconto)||0;
                const dv=s.descontoTipo==="%"?b*(d/100):Math.min(d,b);
                return a+Math.max(0,b-dv);
              }, 0);
              const dg = parseFloat(modal.descontoGlobal)||0;
              const dgVal = modal.descontoGlobalTipo==="%" ? subTotal*(dg/100) : Math.min(dg, subTotal);
              const totalFinal = Math.max(0, subTotal - dgVal);
              return (
                <>
                  {dg > 0 && (
                    <div style={{ display:"flex", justifyContent:"space-between", paddingTop:8 }}>
                      ...
                      <span style={{ color:C.muted, fontSize:12 }}>R$ {subTotal.toFixed(2)}</span>
                    </div>
                  )}
                  {dg > 0 && (
                    <div style={{ display:"flex", justifyContent:"space-between" }}>
                      <span style={{ color:C.red, fontSize:12 }}>Desconto global ({modal.descontoGlobalTipo==="%" ? dg+"%" : "R$ "+dg})</span>
                      <span style={{ color:C.red, fontSize:12 }}>− R$ {dgVal.toFixed(2)}</span>
                    </div>
                  )}
                  <div style={{ display:"flex", justifyContent:"space-between", paddingTop:dg>0?4:10, fontWeight:900 }}>
                    <span style={{ color:C.text }}>TOTAL</span>
                    <span style={{ color:C.accent, fontSize:18 }}>R$ {totalFinal.toFixed(2)}</span>
                  </div>
                </>
              );
            })()}
        ...
        <Modal title={`✏️ Editar OS — ${modalEditar.numero}`} onClose={() => setModalEditar(null)} wide>
          ...
          {editStep===0 && (
            <div>
              ...
              <Field label="CPF / CNPJ">
                <Inp placeholder="000.000.000-00 ou 00.000.000/0000-00" value={modalEditar.clienteCpfCnpj||""} onChange={e=>upEditar("clienteCpfCnpj",e.target.value)}/>
              </Field>
            </div>
          )}
          ...
          {editStep===2 && (
            <div>
              {modalEditar.servicos.map((sv,i) => (
                <div key={i} style={{ background:C.panel, borderRadius:10, padding:12, marginBottom:10, border:`1px solid ${C.border}` }}>
                  ...
                  <div className="form-grid-2" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop:8 }}>
                    <Field label="Desconto por Item">
                      <div style={{ display:"flex", gap:4 }}>
                        <select value={sv.descontoTipo||"R$"} onChange={e=>upEditSvc(i,"descontoTipo",e.target.value)}
                          style={{ ...css.input, width:56, padding:"9px 6px", cursor:"pointer" }}>
                          <option value="R$">R$</option>
                          <option value="%">%</option>
                        </select>
                        <Inp type="number" min="0" placeholder="0" value={sv.desconto||""} onChange={e=>upEditSvc(i,"desconto",e.target.value)} style={{ flex:1 }}/>
                      </div>
                    </Field>
                    <div style={{ display:"flex", flexDirection:"column", justifyContent:"flex-end", paddingBottom:2 }}>
                      {sv.valorUnit ? (() => {
              ...
              <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, padding:"12px 14px", marginTop:14 }}>
                <div style={{ color:C.muted, fontSize:11, fontWeight:700, letterSpacing:1, marginBottom:8 }}>🏷️ DESCONTO GLOBAL DA OS</div>
                <div style={{ display:"flex", gap:4 }}>
                  <select value={modalEditar.descontoGlobalTipo||"R$"} onChange={e=>upEditar("descontoGlobalTipo",e.target.value)}
                    style={{ ...css.input, width:56, padding:"9px 6px", cursor:"pointer" }}>
                    <option value="R$">R$</option>
                    <option value="%">%</option>
                  </select>
                  <Inp type="number" min="0" placeholder="0" value={modalEditar.descontoGlobal||""}
                    onChange={e=>upEditar("descontoGlobal",e.target.value)} style={{ flex:1 }}/>
                </div>
              </div>
        ...
        <Modal title={`Novo Orçamento — ${novaORC.numero}`} onClose={() => { setModalORC(false); setNovaORC(null); }} wide>
          ...
          {stepORC===0 && (
            <div>
              ...
              <Field label="CPF / CNPJ">
                <Inp placeholder="000.000.000-00 ou 00.000.000/0000-00" value={novaORC.clienteCpfCnpj||""} onChange={e=>upORC("clienteCpfCnpj",e.target.value)}/>
              </Field>
            </div>
          )}
          ...
          {stepORC===2 && (
            <div>
              {novaORC.servicos.map((sv,i) => (
                <div key={i} style={{ background:C.panel, borderRadius:10, padding:12, marginBottom:10, border:`1px solid ${C.border}` }}>
                  ...
                  <div className="form-grid-2" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop:8 }}>
                    <Field label="Desconto por Item">
                      <div style={{ display:"flex", gap:4 }}>
                        <select value={sv.descontoTipo||"R$"} onChange={e=>upSvcORC(i,"descontoTipo",e.target.value)}
                          style={{ ...css.input, width:56, padding:"9px 6px", cursor:"pointer" }}>
                          <option value="R$">R$</option>
                          <option value="%">%</option>
                        </select>
                        <Inp type="number" min="0" placeholder="0" value={sv.desconto||""} onChange={e=>upSvcORC(i,"desconto",e.target.value)} style={{ flex:1 }}/>
                      </div>
                    </Field>
                    <div style={{ display:"flex", flexDirection:"column", justifyContent:"flex-end", paddingBottom:2 }}>
                      {sv.valorUnit ? (() => {
              ...
              <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, padding:"12px 14px", marginTop:14 }}>
                <div style={{ color:C.muted, fontSize:11, fontWeight:700, letterSpacing:1, marginBottom:8 }}>🏷️ DESCONTO GLOBAL DO ORÇAMENTO</div>
                <div style={{ display:"flex", gap:4 }}>
                  <select value={novaORC.descontoGlobalTipo||"R$"} onChange={e=>upORC("descontoGlobalTipo",e.target.value)}
                    style={{ ...css.input, width:56, padding:"9px 6px", cursor:"pointer" }}>
                    <option value="R$">R$</option>
                    <option value="%">%</option>
                  </select>
                  <Inp type="number" min="0" placeholder="0" value={novaORC.descontoGlobal||""}
                    onChange={e=>upORC("descontoGlobal",e.target.value)} style={{ flex:1 }}/>
                </div>
              </div>
              ...
              {(() => {
                ...
                const totalFinal = Math.max(0, subTotal - dgVal);
                return (
                  <div style={{ background:"#1e3a5f", borderRadius:10, padding:"14px 16px", marginTop:8 }}>
                    {dg > 0 && (
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                        <span style={{ color:"#93c5fd", fontSize:12 }}>Subtotal dos itens</span>
                        <span style={{ color:"#93c5fd", fontSize:12 }}>R$ {subTotal.toFixed(2)}</span>
                      </div>
                    )}
                    {dg > 0 && (
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
        ...
        <Modal title={`✏️ Editar Orçamento — ${modalEditarORC.numero}`} onClose={() => setModalEditarORC(null)} wide>
          ...
          {editStepORC===0 && (
            <div>
              ...
              <Field label="CPF / CNPJ">
                <Inp placeholder="000.000.000-00 ou 00.000.000/0000-00" value={modalEditarORC.clienteCpfCnpj||""} onChange={e=>upEditORC("clienteCpfCnpj",e.target.value)}/>
              </Field>
          ...
          {editStepORC===2 && (
            <div>
              {modalEditarORC.servicos.map((sv,i) => (
                <div key={i} style={{ background:C.panel, borderRadius:10, padding:12, marginBottom:10, border:`1px solid ${C.border}` }}>
                  ...
                  <div className="form-grid-2" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop:8 }}>
                    <Field label="Desconto por Item">
                      <div style={{ display:"flex", gap:4 }}>
                        <select value={sv.descontoTipo||"R$"} onChange={e=>upEditSvcORC(i,"descontoTipo",e.target.value)}
                          style={{ ...css.input, width:56, padding:"9px 6px", cursor:"pointer" }}>
                          <option value="R$">R$</option>
                          <option value="%">%</option>
                        </select>
                        <Inp type="number" min="0" placeholder="0" value={sv.desconto||""} onChange={e=>upEditSvcORC(i,"desconto",e.target.value)} style={{ flex:1 }}/>
                      </div>
                    </Field>
                    <div style={{ display:"flex", flexDirection:"column", justifyContent:"flex-end", paddingBottom:2 }}>
                      {sv.valorUnit ? (() => {
                        ...
                        return (
                          <div style={{ textAlign:"right" }}>
                            {desc>0 && <div style={{ color:C.red, fontSize:11, textDecoration:"line-through" }}>R$ {bruto.toFixed(2)}</div>}
                            <div style={{ color:C.blue, fontSize:13, fontWeight:700 }}>R$ {liq.toFixed(2)}</div>
                          </div>
                        );
                      })() : null}
                    </div>
              ...
              <div style={{ background:C.panel, border:`1px solid ${C.border}`, borderRadius:10, padding:"12px 14px", marginTop:14 }}>
                <div style={{ color:C.muted, fontSize:11, fontWeight:700, letterSpacing:1, marginBottom:8 }}>🏷️ DESCONTO GLOBAL DO ORÇAMENTO</div>
                <div style={{ display:"flex", gap:4 }}>
                  <select value={modalEditarORC.descontoGlobalTipo||"R$"} onChange={e=>upEditORC("descontoGlobalTipo",e.target.value)}
                    style={{ ...css.input, width:56, padding:"9px 6px", cursor:"pointer" }}>
                    <option value="R$">R$</option>
                    <option value="%">%</option>
                  </select>
                  <Inp type="number" min="0" placeholder="0" value={modalEditarORC.descontoGlobal||""}
                    onChange={e=>upEditORC("descontoGlobal",e.target.value)} style={{ flex:1 }}/>
                </div>
              </div>
...
function Clientes({ clientes, setClientes, ordens, pendenciaEdicao, onConsumirPendencia }) {
  const FORM_VAZIO = { nome:"", tel:"", whatsapp:"", email:"", cpfCnpj:"", endereco:"", cidade:"", obs:"", motos:[] };
  const [modal,    setModal]    = useState(false);
  const [editando, setEditando] = useState(null);
  const [form,     setForm]     = useState(FORM_VAZIO);
  const [busca,    setBusca]    = useState("");
  ...
  useEffect(() => {
    ...
    setEditando(cli);
    setForm({ nome:cli.nome||"", tel:cli.tel||"", whatsapp:cli.whatsapp||"", email:cli.email||"", cpfCnpj:cli.cpfCnpj||"", endereco:cli.endereco||"", cidade:cli.cidade||"", obs:cli.obs||"", motos:cli.motos||[] });
    setModal(true);
    onConsumirPendencia();
  }, [pendenciaEdicao]);
    ...
    <div className="fade-in">
      ...
      {modal && (
        <Modal title={editando ? "✏️ Editar Cliente" : "👤 Novo Cliente"} onClose={() => { setModal(false); setEditando(null); }}>
          ...
          <Field label="CPF / CNPJ">
            <Inp value={form.cpfCnpj||""} placeholder="000.000.000-00 ou 00.000.000/0000-00" onChange={e => setForm(f => ({ ...f, cpfCnpj: e.target.value }))}/>
          </Field>
...
function BuscaGlobal({ ordens, orcamentos, clientes, estoque, onNavigate, onPrint }) {
  ...
  return (
    <div ref={wrapRef} style={{ position:"relative", flex:1, maxWidth:480 }}>
      ...
      {aberto && termo.length >= 2 && dropPos && ReactDOM.createPortal(
        <div style={{
